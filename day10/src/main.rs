...
fn main() {
    ...
    for (x, y) in trailheads {
        let mut route = vec![(x, y)];
        if let Some(paths) = trailpaths(&trailmap, &mut route) {
            result.extend(paths);
        }
    }
...
fn trailpaths(map: &[Vec<u8>], route: &mut [(usize, usize)]) -> Option<Vec<Vec<(usize, usize)>>> {
    let (x, y) = route.last().copied().expect("Route is empty");

    let h = map[x][y];
    if h == 9 {
        return Some(vec![route.to_owned()]);
    }
    ...
    if x > 0 && map[x - 1][y] == h + 1 {
        ...
        route.push((x - 1, y));
        if let Some(paths) = trailpaths(map, &mut route) {
            result.extend(paths);
        }
    }
